...
| `PROXY_ALLOW` | 下記6つ | Ollamaへ中継を許可するパス |
...
- **許可リスト方式のプロキシ** — Ollamaへ中継するのは `/api/tags`・`/api/version`・`/api/show`・`/api/chat`・`/api/generate`・`/api/embeddings` のみ（`/api/show` はモデルの能力を読むだけの参照系）。モデルの削除やpullといった管理系エンドポイントは403で拒否します
...
| ON/OFF型 | `thinking` はあるが `{{ .ThinkLevel }}` は無い | `think:false` で思考が止まる。`"low"`/`"medium"`/`"high"` を送っても `true` と同じ | 既定では隠す（下記） |
...
既定では**レベル型だけをUIに出します**（`THINK_SHOW`）。ON/OFF型の思考は、モデルがVRAMに収まりきらない構成だと回答が始まるまで数分かかることがあり、タイムアウトが先に作動して実用にならないためです。余裕のあるGPUで使う場合は `THINK_SHOW` に `"bool"` を加えてください。
